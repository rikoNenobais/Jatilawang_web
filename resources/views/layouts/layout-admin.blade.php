...
<div class="min-h-screen flex">
    ...
    <aside class="hidden md:flex md:flex-col md:w-64 bg-white shadow-lg border-r border-slate-100">
        {{-- Brand --}}
        <div class="p-4 border-b border-slate-100 bg-gradient-to-r from-emerald-600 to-teal-500">
        ...
        <nav class="flex-1 p-3 space-y-1 text-sm">
            ...
            @foreach($navItems as $item)
                ...
                class="group flex items-center gap-3 px-3 py-2 rounded-lg
                    ...
                    <svg class="w-5 h-5 {{ $active ? 'text-emerald-500' : 'text-slate-400 group-hover:text-slate-500' }}"
    ...
    <main class="flex-1 flex flex-col">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <form class="inline" method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button
                            class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-semibold rounded-md text-white bg-amber-500 hover:bg-amber-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500">
                            Logout
                        </button>
                    </form>
                </div>
        ...
        <section class="flex-1 py-6">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                @if (session('error'))
                    <div
                        class="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 flex items-start gap-2">
                        <svg class="w-5 h-5 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                            stroke-width="1.8">
                            <path stroke-linecap="round" stroke-linejoin="round"
