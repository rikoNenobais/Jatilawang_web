...
@section('content')
                        ...
                        @auth
                            @if(Auth::user()->role === 'admin')
                                ...
                                <h1 class="mt-3 leading-[0.95] tracking-tight">
                                    <span class="block text-[52px] md:text-[84px] lg:text-[80px] font-light">Dashboard</span>
                                    <span class="block -mt-2 text-[72px] md:text-[100px] lg:text-[90px] font-extrabold">Admin</span>
                                </h1>
                                <p class="mt-6 max-w-xl text-emerald-100 text-base md:text-lg">
                                    Kelola produk, pantau peminjaman, dan lihat laporan keuangan dari dashboard admin.
                                </p>
                                <a href="{{ route('admin.dashboard') }}"
                                   class="mt-8 inline-flex h-12 items-center justify-center rounded-xl border border-white/70 px-6 text-white font-semibold hover:bg-white/10 transition-colors">
                                    Dashboard Admin
                                </a>
                            @else
    ...
    @auth
        @if(Auth::user()->role !== 'admin')
            {{-- Tampilkan slider hanya untuk non-admin --}}
    ...
    @auth
        @if(Auth::user()->role !== 'admin')
            {{-- Tampilkan produk hanya untuk non-admin --}}
